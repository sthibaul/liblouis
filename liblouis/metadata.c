      ...
      while (_lou_getALine(&info))
	{
	  if (info.linelen == 0);
	      ...
	      if (info.linelen >= 2 && (info.line[1] == '+' || (!activeOnly && info.line[1] == '-')))
		{
		  ...
		  size_t valSize = 0;
		  info.linepos = 2;
		  if (info.linepos < info.linelen && isValidChar((char) info.line[info.linepos]))
		    {
		      key = &info.line[info.linepos];
		      keySize = 1;
		      info.linepos++;
		      while (info.linepos < info.linelen && isValidChar((char) info.line[info.linepos]))
			{
			  keySize++;
			  info.linepos++;
			}
		      if (info.linepos < info.linelen && info.line[info.linepos] == ':')
			{
			  ...
			  if (info.linepos < info.linelen
			      && (!active || isValidChar((char) info.line[info.linepos])))
			    {
